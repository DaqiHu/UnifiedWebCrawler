...
def run_crawl(service: CrawlService, target: str, related_limit: int) -> None:
...
def run_category_sync(service: CrawlService, category_id: str) -> None:
...
def run_full_sync(service: CrawlService) -> None:
...
def render_page() -> None:
    ...
    category_counts = service.fetch_category_counts(SOURCE, internships_only=True)
    category_options = {
        item["competencyType"]: f'{item["competencyTypeName"]} ({item["count"]})'
        for item in category_counts
    }

    jobs_df = storage.jobs_dataframe(SOURCE)
    bootstrap_key = "mihoyo_auto_bootstrapped"
    if jobs_df.empty and bootstrap_key not in st.session_state:
    ...
    with st.sidebar:
        render_back_home()
        st.title("miHoYo 校招岗位")
        st.caption("招聘岗位抓取与分析页。")
        crawl_mode = st.radio(
            "抓取模式",
            options=["单岗位 + 相似岗位", "同类别全量（仅实习）", "全站全量（仅实习）"],
        )
        target = st.text_input("岗位 URL 或岗位 ID", value=connector.default_target)
        ...
        if st.button("执行抓取 / 同步", type="primary", use_container_width=True):
            try:
                if crawl_mode == "单岗位 + 相似岗位":
                    run_crawl(service, target, related_limit)
                elif crawl_mode == "同类别全量（仅实习）":
                    run_category_sync(service, selected_category_id)
                else:
                    run_full_sync(service)
            except ConnectorError as error:
                st.error(str(error))
    ...
    catalog_columns[1].markdown(
        """
        <div class="source-card">
            <h4>ARC Raiders 武器页</h4>
            <div>类型: 武器资料</div>
            <div>状态: 已拆到独立页面</div>
            <div style="margin-top: 0.6rem;">与岗位分析完全独立，避免把不同语义的数据揉在一套视图里。</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    catalog_columns[2].markdown(
        ...
        <div class="source-card">
            <h4>更多独立需求</h4>
            <div>类型: 待扩展</div>
            <div>状态: 首页路由可继续追加</div>
            <div style="margin-top: 0.6rem;">每个需求一个页面目录，主入口只保留导航职责。</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with detail_tab:
        if jobs_df.empty:
            ...
            st.subheader("相似岗位")
            related_df = storage.related_jobs_dataframe(SOURCE, selected_job_id)
